...
import type { MixParentSource, MixSelection } from '@/components/MixPicker';
...
export interface ParsedMixAxisSegment {
  /** Parent breed or temperament category label when recorded in the mix string. */
  source?: string;
  detail: string;
}
...
/** Build a client-safe mix label without parent trait blurbs in the stored string. */
export function formatMixBreedLabelCompact(selection: MixSelection): string {
  return `${formatMixTitle(normalizeMixSelection(selection))} mix`;
}
