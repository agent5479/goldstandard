...
function ParentReference({ label, breed }: ParentReferenceProps) {
  ...
  return (
    <div className="exam-breed-ref-block">
      ...
      <p className="exam-breed-ref-type">{getBreedClientMixTraitLabel(breed.name)}</p>
    ...
    return (
      <div className="exam-mix-parent">
        ...
        <div className="exam-breed-chip">
          ...
          {selected && (
            <span className="exam-breed-cat">{getBreedClientMixTraitLabel(selected.name)}</span>
          )}
          ...
          {matches.map((b) => (
            <li key={b.name} role="option" aria-selected={false}>
              <button type="button" className="exam-breed-result" onClick={() => onPick(b)}>
                ...
                <span className="exam-breed-cat">{getBreedClientMixTraitLabel(b.name)}</span>
                <span className="exam-breed-ref-snippet">{getBreedFullProfile(b).personality.split('.')[0]}.</span>
              </button>
            </li>
          ))}
        </ul>
  ...
  return (
    <section className="exam-step" id="exam-mix" aria-labelledby="exam-mix-heading">
      ...
      <p className="exam-step-desc">
        Mixes inherit unevenly — the build can come from one side and the personality from another, or from
        somewhere else entirely. Pick up to three parent breeds (or your best guess), then tell us which traits
        your dog actually ended up with.
      </p>
                  ...
                  <div className="exam-trait-options mix-trait-card-grid">
                    {selectedParents.map(({ source, breed }) => (
                      <TraitOption
                        ...
                        parentSlot={source}
                        name={breed.name}
                        axisDetail={getBreedMixAxisProfile(breed, axis.key, 'client')}
